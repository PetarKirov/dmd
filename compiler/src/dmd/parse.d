...
class Parser(AST, Lexer = dmd.lexer.Lexer) : Lexer
{
  ...
  final:
            ...
            Lprot:
                {
                    ...
                    // visibility to
                    IdentifierAtLoc[] pkg_prot_idents;
     ...
     */
    private IdentifierAtLoc[] parseQualifiedIdentifier(const(char)* entity)
    {
        IdentifierAtLoc[] qualified;
        ...
        do
        {
            ...
            Identifier id = token.ident;
            qualified ~= makeIdentifierAtLoc(id, token.loc);
                ...
                default:
                    {
                        ...
                        auto param = new AST.Parameter(loc, storageClass | STC.parameter, at, makeIdentifierAtLoc(ai, identloc), ae, null);
                        if (udas)
                        {
                            auto a = new AST.Dsymbols();
                            auto udad = new AST.UserAttributeDeclaration(udas, a);
        ...
        L1:
            ...
            if (token.value == TOK.colon)
            {
                do
                {
                    ...
                    s.names.push(makeIdentifierAtLoc(name, nameloc));
                    s.aliases.push(makeIdentifierAtLoc(_alias, aliasloc));
                }
    ...
    ///Ditto
    AST.Type parseType(Identifier* pident = null, Loc* pidentLoc = null,
                       STC stc = STC.none)
    {
        ...
        if (pidentLoc)
            *pidentLoc = token.loc;
        int alt = 0;
        t = parseDeclarator(t, alt, pident, pidentLoc);
     ...
     *  pident       = set to Identifier if there is one, null if not
     *  pidentLoc    = set to the location of the identifer
        ...
        case TOK.identifier:
            {
                ...
                parameterList.parameters.push(new AST.Parameter(loc, STC.parameter, t, makeIdentifierAtLoc(token.ident, token.loc), null, null));
    ...
    private Foreach parseForeach(alias Foreach)(Loc loc, AST.Dsymbol* pLastDecl)
    {
        ...
        Larg:
            auto p = new AST.Parameter(aloc, storageClass, at, makeIdentifierAtLoc(ai, aloc), null, null);
            parameters.push(p);
            if (token.value == TOK.comma)
            {
                nextToken();
    ...
    Lwhile:
        ...
        {
            auto ai = makeIdentifierAtLoc(token.ident, token.loc);
    ...
    private AST.Expression parsePostExp(AST.Expression e)
    {
        while (1)
        {
            ...
            case TOK.minusMinus:
                e = new AST.PostExp(EXP.minusMinus, e.loc, e);
                break;
